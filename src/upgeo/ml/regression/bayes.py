...
from upgeo.ml.regression.linear import LinearRegressionModel, RidgeRegression
...
class EMBayesRegression(LinearRegressionModel):
    ...
    def _get_hyperparams(self):
        ...
        '''
        return np.asarray([self._alpha, self._beta])
...
if __name__ == '__main__':
    from upgeo.eval.multitask.base import load_data, make_rbf
...
#    rbf, Xrbf = make_rbf(X, kernel=GaussianBasis(0.1), ratio=0.1)
#    
#    #X = np.random.rand(15, 20)
#    #y = 0.5*X[:,2] + 0.25*X[:,7] + 0.1*X[:,4] + 2.5*X[:,14] + np.random.randn(15)*0.5
#    
#    reg_model = EBChenRegression(alpha0=0.1, weight_bias=False)
#    reg_model.fit(X[0][0:40],Y[0][0:40])
#    print 'alpha={0}, beta={1}'.format(reg_model.alpha, reg_model.beta)
#    reg_model = EMBayesRegression(weight_bias=False)
#    reg_model.fit(X[0],Y[0])
#    print 'alpha={0}, beta={1}'.format(reg_model.alpha, reg_model.beta)
#    
#    savetxt('/home/marcel/test_data.csv', np.c_[X[1][0:30],Y[1][0:30]], delimiter=',')
    
#    from upgeo.eval.multitask.base import load_data, make_rbf
#    from upgeo.ml.multitask.base import flatten_data, compound_data
#
#    filename = '/home/marcel/datasets/multilevel/ilea/exam_london.csv'
#    task_key = 'school'
#    task_fields = ['school_mixed', 'school_boys', 'school_girls', 
...
#    
#    X,Y,Z = load_data(filename, task_key, task_fields, target_field) 
#    #rbf = make_rbf(X, Z, kernel=GaussianBasis(0.25))
#    #covars = flatten_data(compound_data(X,Z))
#    covars = flatten_data(compound_data(X, Z))
#    targets = flatten_data(Y)
#    
#    print 'shape={0}'.format(covars.shape)
#    
#    n = 8000
#    
#    
#    t = time.clock()
#    for i in xrange(n):
#        w = np.dot(np.linalg.pinv(covars), targets)
#    print 'pinv={0}'.format(time.clock()-t)
#    print w
#    
#    t = time.clock()
#    for i in xrange(n):
#        w = np.linalg.solve(np.dot(covars.T, covars), np.dot(covars.T, targets))
#    print 'solve={0}'.format(time.clock()-t)
#    print w
#
#    
#    t = time.clock()
#    for i in xrange(n):
#        model = MapBayesRegression(weight_bias=True)
#        model.fit(covars, targets)
#    print 'time bayes={0}'.format(time.clock()-t)
#    #print model.weights
...
#        model.fit(covars, targets)
#    print 'time ridge={0}'.format(time.clock()-t)
#    #print model.weights
#    print model.intercept
#    
#    t = time.clock()
...
#        model = Ridge()
#        model.fit(covars, targets)
#    print 'time ridge scikit={0}'.format(time.clock()-t)
#    #print model.coef_
#    #print model.intercept_
#    
#    t = time.clock()
#    for i in xrange(n):
#        model = FastBayesRegression(weight_bias=True)
#        model.fit(covars, targets)
#    print 'fast bayes={0}'.format(time.clock()-t)
#    #print model.weights
#    print model.intercept
#    
#    t = time.clock()
#    for i in xrange(n):
#        model = GaussianBayesRegression()
#        model.fit(covars, targets)
#    print 'gauss bayes={0}'.format(time.clock()-t)
#    print model.weights
#    print model.intercept
    
    
    
    #reg_model = MapBayesRegression(alpha=7.433770177780715, beta=1.686562893575060)
    #reg_model = EMBayesRegression(alpha0=7.433770177780715, beta0=1.686562893575060)
    #reg_model = EBChenRegression(0.1, 1, weight_bias=False)
    #reg_model.fit(covars, targets)
    
    #yhat = reg_model.predict(covars)
    
    #print reg_model.alpha
    #print reg_model.beta
    #print reg_model.log_evidence
    #print reg_model.weights
    #print reg_model.intercept
    
    #print mspe(targets, yhat)
    
    #covars = np.c_[np.ones(len(covars)), covars]
    #likel_fun = EvidenceLikelihood(covars, targets, False)
    
    #print likel_fun(reg_model.alpha, reg_model.beta)
    
    #print likel_fun.w 
    #print likel_fun.invA
    #print likel_fun.A

    #np.random.seed(1000)
    #X = np.c_[np.random.rand(3,4)]
    #y = 1.45 + X[:,0]*1.45 + X[:,1]*1.45 + X[:,2]**2*1.45 + X[:,3]*1.45 + 0.5*np.random.randn(3)
        
    #likel_fun = AlphaEvidenceLikelihood(X, y, 1)
    #fit = lambda x: x**2
    #grad = lambda x: 2*x
    

    #print likel_fun.gradient(3)
    
    #print check_gradient(likel_fun, likel_fun.gradient, 1)
    #print check_gradient(fit, grad, 10)
    #print 'start'
    #print fmin_cg(likel_fun, 3)
