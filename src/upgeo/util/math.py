...
def sumln(a, axis=None, dtype=None):
    '''
    '''
    a = np.asanyarray(a)
    if axis == None:
        ...
        res = np.apply_along_axis(_sumln_array, axis, a)
    return np.asanyarray(res, dtype)

def diffln(a, axis=None, dtype=None):
    '''
    '''
    a = np.asanyarray(a)
    if axis == None:
        ...
        res = np.apply_along_axis(_diffln_array, axis, a)
    return np.asanyarray(res, dtype)
...
def _diffln_array(a):
    ...
    n = len(a)
    if n > 1:
        res = _diffln_pair(res, _sumln_array(a[1:n]))
    return res
...
def geninv(a):
    ...
    dA = np.diag(A)
    tol = np.min(dA[dA>0])*1e-9 #check if indexing works
    L = np.empty(A.shape)
    r = 0
    for k in xrange(n):
        L[k:n,r] = A[k:n,k] - np.dot(L[k:n, 0:r], L[k,0:r].T)
        if L[k,r] > tol:
            L[k,r] = np.sqrt(L[k,r])
            if k < n:
    ...
    print (time.clock()-t)
    L = L[:,0:r]
    #compute the generalized inverse matrix of a
    t = time.clock()
    M = np.linalg.inv(np.dot(L.T, L))
    print (time.clock()-t)
    t = time.clock()
    if transposed == True:
        Z = np.dot(np.dot(np.dot(np.dot(a.T, L), M), M), L.T)
    else:
        #Z = np.dot(L, np.dot(M, np.dot(M, np.dot(L.T, a.T))))
        Z = np.dot(np.dot(np.dot(np.dot(L, M), M), L.T), a.T)
    print (time.clock()-t)
    return Z
...
if __name__ == '__main__':
    ...
    Z = np.asarray([[3.0, 2.1, 0.8], [1.0, 0.451, 0.1345], [1.013, 34.3, 3.8], [2.30, 2.0355, 1.8]])
    ...
    t = time.clock()
    A = np.dot(X.T, X)
    print 'vdot={0}'.format(time.clock()-t)
    
    Z = np.asmatrix(X)
    
    t = time.clock()
    B = Z.T*Z
    print 'mdot={0}'.format(time.clock()-t)
    ...
    for i in xrange(n):
        B = geninv(X)
    print 'geninv={0}'.format(time.clock()-t)
