...
def eval_sgp(train, test, kernel, k, use_meta_features, use_background_data):
    ...
    if use_background_data:
        ...
        selector = KMeansSelector(k)
#    selector = KMeansSelector(k, True) #include targets
...
if __name__ == '__main__':
    nfolds = 10
    fold_filename = '/home/marcel/datasets/multilevel/synth_gmm_splitz/gmm_{0}_indexes.mat'
    
    filename = '/home/marcel/datasets/multilevel/synth_gmm/mt_gmm_norm3.csv'
    task_key = 'model'
    task_fields = ['model']
    target_field = 'pga' 
    ...
    for i in xrange(nfolds):
        ...
        kernel = ARDSEKernel(np.log(1)*np.ones(3), np.log(1)) + NoiseKernel(np.log(1))
        #kernel = ARDSEKernel(np.log(1)*np.ones(3), np.log(1)) + SqConstantKernel(np.log(1)) * LinearKernel() + NoiseKernel(np.log(1))
